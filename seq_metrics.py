...
def calculate_sequence_properties(seq: str) -> dict:
    """Calculate basic biochemical properties of a protein sequence."""
    analysis = ProteinAnalysis(seq)
    aa_content = analysis.get_amino_acids_percent()

    charge_fraction = sum(aa_content.get(aa, 0) for aa in ["K", "R", "D", "E"])
...
def calculate_global_similarity(ref_seq: str, seq: str) -> float:
    """Calculate global sequence identity relative to reference sequence."""
...
def calculate_active_site_similarity(
    ...
    """Calculate active site similarity score using substitution matrix with gap penalties."""
    score = 0
    in_gap = False
    ...
    for ref_aa, target_aa in zip(ref_seq, target_seq):
        if ref_aa == "-" or target_aa == "-":
            ...
            try:
                ...
                score += -4  # penalty for unknown residues
...
def map_active_site_positions(alignment, reference_positions: list) -> list:
    """
    Map the active site positions from ungapped reference numbering
    to positions in the alignment (gapped).
    """
...
def get_seq_metrics(
    ...
    """
    Gather sequence metrics from an aligned FASTA file (e.g., Clustal Omega output),
    including biochemical properties, global similarity, active site scores,
    and longest repeats for k = 1..max_repeat_k.
    """
    ...
    for seq_record in alignment:
        ...
        # Remove gaps for biochemical properties
        ...
        if not clean_seq:
            ...
            seq_props = calculate_sequence_properties(clean_seq)
        ...
        # Longest repeats for k=1..max_repeat_k
        repeats = {
